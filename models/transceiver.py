...
# ADDED (This work):
    ...
    def __init__(self, num_layers,
        ...
        # 3개의 latent branch projection
        self.z_sem_proj = nn.Linear(d_model, d_model)
        self.z_rob_proj = nn.Linear(d_model, d_model)
        self.z_snr_proj = nn.Linear(d_model, d_model)

        # SNR (또는 noise variance)를 d_model로 올리는 MLP
        self.z_snr_mlp = nn.Sequential(
            nn.Linear(1, d_model),
            nn.ReLU(inplace=True),
            nn.Linear(d_model, d_model)
        )

        # z_sem을 decoder에 더할 때 쓸 scale (학습 가능하게 둘 수도 있음)
        self.sem_weight = nn.Parameter(torch.tensor(1.0))

        self.gate_net = nn.Sequential(
        nn.Linear(d_model, 2)  # [B,T,d_model] → [B,T,2] (w_sem, w_rob 용)
        )
    ...
    #ADDED (This work):
        ...
        return:
          z_total: 채널에 보낼 최종 latent [B, T, d_model]
          z_dict: {'z_sem', 'z_rob', 'z_snr'}
        """
        B, T, d_model = enc_output.size()

        # n_var를 [B,T,1]로 확장
        if not torch.is_tensor(n_var):
            n_tensor = torch.tensor([[[float(n_var)]]],
                                    device=enc_output.device,
                                    dtype=enc_output.dtype)
        else:
            n_tensor = n_var.view(1, 1, 1).to(enc_output.device)

        snr_input = n_tensor.expand(B, T, 1)      # [B,T,1]
        snr_feat = self.z_snr_mlp(snr_input)      # [B,T,d_model]

        # 3개 branch
        z_sem = self.z_sem_proj(enc_output)       # [B,T,d_model]
        z_rob = self.z_rob_proj(enc_output)       # [B,T,d_model]
        z_snr = self.z_snr_proj(enc_output) + snr_feat  # SNR-aware branch

        # 일단 간단히 평균으로 합쳐서 채널로 보낼 latent 생성
        z_total = (z_sem + z_rob + z_snr) / 3.0
